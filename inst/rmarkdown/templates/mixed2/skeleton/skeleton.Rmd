...
Data on a range range of Korean pop ("K-pop") and Jazz artists and their tracks was collected through from the Spotify API. You will have a sample of this data to work with.

## Data dictionary

|variable                 |class     |description |
|:---|:---|:-----------|
|id | numeric | Unique ID (arbirtrary) |
...
## Part 2: Tables and calculat

1. Using the `training_data`, create a table of `genre` by `mode_name`. I.e., have `genre` on the rows and `mode_name` on the columns.
...
3. Using the `training_data`, calcule the probability that a song chosen at random from your sample is a Jazz song in a major key.
...
Note: You do not have to any further model tweaking other than the instructions provided.
...
2. Using the `training_data`, fit an appropriate __mixed effects model__ (use the `lme4` package) to predict whether or not a song is K-pop based on `artist_popularity`, `danceability`, `valence` with a random effect for `artist_name`. (This make take some time to run)

* Name your model: `model_2`.
* __Important:__ Add the following argument to your model: `nAGQ=0`.
* It is okay if you get a message about it failing to converge. You can ignore that for our purposes.
* Run `summary()` on your model.
...
1. Uncomment the code below to add predictions for all your models to your `testing_data`.
